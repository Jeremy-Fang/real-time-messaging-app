import React from 'react'

const RecentMessageCard = ({ imgURL, members, messages, idMap, i, selected, onCustomClick }) => {
    const getTimePast = (date, metric) => {
        const milliMap = {
            'seconds': 1000,
            'minutes': 60 * 1000,
            'hours': 60 * 60 * 1000,
            'days': 24 * 60 * 60 * 1000
        }

        return (Date.now() - new Date(date)) / milliMap[metric]
    }

    const getLastMessageTime = (date) => {
        if (Math.floor(getTimePast(date, 'days')) > 0) {
            return new Date(date).toDateString().split(' ').splice(1).join(' ')
        }

        if (Math.floor(getTimePast(date, 'hours')) > 0) {
            return `${Math.floor(getTimePast(date, 'hours'))} hours ago`
        }

        if (Math.floor(getTimePast(date, 'minutes')) > 0) {
            return `${Math.floor(getTimePast(date, 'minutes'))} minutes ago`
        }

        return 'now'
    }

    return (
        <>
            <div key={i} className={`recent-message-card ${selected == i ? 'selected': ''}`} onClick={onCustomClick}>
                <img src={imgURL ? imgUrl : 'data:image/png;base64,'} />
                <div className='text-group'>
                    <span className='recent-message-name'>{
                        (members.length == 2 ?
                            members.filter(member => {
                                return member != sessionStorage.getItem('uid')
                            })
                            :
                            members).map(member => {
                                return idMap[member]
                            }).join(', ')
                    }</span>
                    <span className='last-message'>{
                        messages.length > 0 ?
                            messages[messages.length - 1].message
                            :
                            null
                    }</span>
                </div>
                <div className='last-message-time'>{
                    messages.length == 0 ?
                        null
                        :
                        getLastMessageTime(messages[messages.length - 1].time)
                }</div>
            </div>
        </>
    )
}

export default RecentMessageCard