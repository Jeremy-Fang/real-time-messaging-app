import React from 'react'

const RecentMessageCard = ({ imgURL, name, lastMessage, changeSelectedConv }) => {
    return (
        <>
            <div className='recent-messages-container' onClick={ changeSelectedConv }>
                <div className='entry'>
                    <div className='message-group'>
                        <img src={imgURL ? imgUrl: 'data:image/png;base64,'} />
                        <div className='text-group'>
                            <span className='messager-name'>{name}</span>
                            <span className='last-message'>{lastMessage == null ? '': lastMessage.message}</span>
                        </div>
                    </div>
                    <div className='last-message-time'>
                        {lastMessage == null ? '': lastMessage.time}
                    </div>
                </div>
            </div>
        </>
    )
}

export default RecentMessageCard